...
const TopStoriesPosts = ({ post }) => {
  ...
  return (
    ...
    <div className="cursor-pointer min-h-fit relative pb-10 -mt-20 md:-mt-5 group mb-5">
      <img
        className=" rounded-lg h-56 md:h-60 w-full object-cover  lg:h-40 group-hover:scale-105 transition-transform duration-200 ease-in-out"
        src={post.image}
        alt=""
      />
      <h1 className="text-xl font-medium mt-5">{post.title}</h1>
      <p className="mt-5 pb-10">{truncate(post.subtitle, 100)}</p>
      <div className="flex space-x-2 absolute bottom-0 left-0">
        <img
